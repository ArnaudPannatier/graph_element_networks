...
class PoissonSquareRoomOutDataset(Dataset):
    ...
    def __init__(self, file_path):
        ...
        self.DATA = [
            [f[str(key) + "-IN"][:], f[str(key) + "-OUT"][:]] for key in self.NAMES
        ]
    ...
    def __getitem__(self, idx):
        if type(idx) is not int:
            idx = idx.item()
        return (
            torch.FloatTensor(self.DATA[idx][0]),
...
class PoissonSquareRoomInpDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if type(idx) is not int:
            idx = idx.item()
        with open(
            os.path.join(self.dir_path, "desc_" + str(idx) + ".json"), "r"
        ) as infile:
        ...
        for room in context:
            ...
            for box in room["unnorm_boxes"]:
                ...
                inputs.append([lx, (ly + Ly) / 2.0, 0, heat, 1])
                inputs.append([Lx, (ly + Ly) / 2.0, 0, heat, 1])
                inputs.append([(lx + Lx) / 2.0, ly, 0, heat, 1])
                inputs.append([(lx + Lx) / 2.0, Ly, 0, heat, 1])

                inputs.append([(lx + Lx) / 2.0, (ly + Ly) / 2.0, 0, heat, 1])

            # Encode border
            ext_temp = room["exterior_temp"]
            num_border = inputs_per_room - len(inputs)
            delta_l = 4.0 / num_border
        ...
        return (all_inputs[:, :, :2].contiguous(), all_inputs[:, :, 2:].contiguous())
